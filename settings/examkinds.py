...
class Ui_MainWindow(QMainWindow):
    ...
    def setupUi(self, MainWindow):
        ...
        self.editName.setGeometry(QtCore.QRect(300, 220, 80, 20))
        ...
        self.tbKinds.setGeometry(QtCore.QRect(0, 0, 400, 200))
        ...
        self.editID.setGeometry(QtCore.QRect(100, 220, 80, 20))
        ...
        self.tbKinds.setAlternatingRowColors(True)  # 使表格颜色交错显示
        self.tbKinds.verticalHeader().setVisible(False)  # 隐藏垂直标题
        self.query()  # 窗体加载时显示所有数据
        self.tbKinds.itemClicked.connect(self.getItem)  # 获取选中的单元格数据
    ...
    # 查询考试类别信息，并显示在表格中
    def query(self):
        self.tbKinds.setRowCount(0)  # 清空表格中的所有行
        result = service.query("select * from tb_examkinds")  # 调用服务类中的公共方法执行查询语句
        row = len(result)  # 取得记录个数，用于设置表格的行数
        self.tbKinds.setRowCount(row)  # 设置表格行数
        self.tbKinds.setColumnCount(2)  # 设置表格列数
        # 设置表格的标题名称
        self.tbKinds.setHorizontalHeaderLabels(['类别编号', '类别名称'])
        for i in range(row):  # 遍历行
            for j in range(self.tbKinds.columnCount()):  # 遍历列
                data = QTableWidgetItem(str(result[i][j]))  # 转换后可插入表格
                self.tbKinds.setItem(i, j, data)  # 设置每个单元格的数据
    ...
    def add(self):
        ...
        if kindID != "" and kindName != "":  # 判断类别编号和类别名称不为空
            if self.getName(kindName) > 0:
                ...
                QMessageBox.information(None, '提示', '您要添加的考试类别已经存在，请重新输入！', QMessageBox.Ok)
            else:
                # 执行添加语句
        ...
        try:
            if self.select != "":  # 判断是否选择了要修改的数据
                ...
                if kindName != "":  # 判断类别名称不为空
                    if self.getName(kindName) > 0:
                        self.editName.setText("")
                        QMessageBox.information(None, '提示', '您要修改的考试类别已经存在，请重新输入！', QMessageBox.Ok)
                    else:
                        # 执行修改操作
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.label_3.setText(_translate("MainWindow", "类别名称："))
        self.label_2.setText(_translate("MainWindow", "类别编号："))
        ...
        item.setText(_translate("MainWindow", "类别编号"))
        item = self.tbKinds.horizontalHeaderItem(1)
        item.setText(_translate("MainWindow", "类别名称"))
