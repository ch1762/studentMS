...
class Ui_MainWindow(QMainWindow):
    ...
    def setupUi(self, MainWindow):
        ...
        self.tbSubject.setGeometry(QtCore.QRect(0, 0, 400, 200))
        ...
        self.tbSubject.setAlternatingRowColors(True)  # 使表格颜色交错显示
        self.tbSubject.verticalHeader().setVisible(False)  # 隐藏垂直标题
        self.query()  # 窗体加载时显示所有数据
        self.tbSubject.itemClicked.connect(self.getItem)  # 获取选中的单元格数据
    ...
    def getItem(self, item):
        if item.column() == 0:  # 如果单击的是第一列
            ...
            self.editID.setText(self.select)  # 显示在科目编号文本框中
    ...
    def add(self):
        ...
        if subID != "" and subName != "":  # 判断科目编号和科目名称不为空
            if self.getName(subName) > 0:
                ...
                QMessageBox.information(None, '提示', '您要添加的科目已经存在，请重新输入！', QMessageBox.Ok)
            else:
                # 执行添加语句
    ...
    def edit(self):
        try:
            if self.select != "":  # 判断是否选择了要修改的数据
                subName = self.editName.text()  # 记录修改的科目名称
                if subName != "":  # 判断科目名称不为空
                    if self.getName(subName) > 0:
                        self.editName.setText("")
                        QMessageBox.information(None, '提示', '您要修改的科目已经存在，请重新输入！', QMessageBox.Ok)
                    else:
                        # 执行修改操作
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.label_3.setText(_translate("MainWindow", "科目名称："))
        self.label_2.setText(_translate("MainWindow", "科目编号："))
